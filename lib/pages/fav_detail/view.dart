...
class _FavDetailPageState extends State<FavDetailPage> with GridMixin {
                    ...
                    ? Column(
                        ...
                        children: [
                          ...
                          FloatingActionButton.small(
                            heroTag: 'addToMusic',
                            onPressed: () => _showImportToMusicSheet(context),
                            tooltip: '添加到音乐播放',
  ...
  /// 显示导入到音乐播放列表的选择弹窗
  void _showImportToMusicSheet(BuildContext context) {
    final state = _favDetailController.loadingState.value;
    if (state is! Success ||
        state.dataOrNull == null ||
        state.dataOrNull!.isEmpty) {
      SmartDialog.showToast('收藏夹为空');
      return;
    }

    final items = state.dataOrNull!
        .where((item) => item.attr != 9) // 排除失效视频
        .toList();

    if (items.isEmpty) {
      SmartDialog.showToast('没有可导入的视频');
      return;
    }
    ...
    showModalBottomSheet(
      ...
      isScrollControlled: true,
      useSafeArea: true,
      builder: (context) => _ImportToMusicSheet(items: items),
    );
  }
}

/// 导入到音乐播放列表的选择弹窗
class _ImportToMusicSheet extends StatefulWidget {
  final List<FavDetailItemModel> items;

  const _ImportToMusicSheet({required this.items});

  @override
  State<_ImportToMusicSheet> createState() => _ImportToMusicSheetState();
}

class _ImportToMusicSheetState extends State<_ImportToMusicSheet> {
  ...
  void _importSelected() {
    if (_selectedIndices.isEmpty) {
      SmartDialog.showToast('请选择要导入的视频');
      return;
    }
    ...
    SmartDialog.showToast('已添加 $count 个视频到音乐播放列表');
    Get.back();
  }
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const Spacer(),
                Text(
                  '已选 ${_selectedIndices.length}/${widget.items.length}',
              ...
              children: [
                ...
                Expanded(
                  child: FilledButton(
                    ...
                    child: Text('导入选中 (${_selectedIndices.length})'),
                  ),
                ),
              ],
            ),
