...
class _VideoDetailPageVState extends State<VideoDetailPageV>
  ...
  Widget _moreBtn(Color color, {List<Shadow>? shadows}) => PopupMenuButton(
    ...
    itemBuilder: (BuildContext context) => <PopupMenuEntry>[
      ...
      if (!videoDetailController.isFileSource)
        PopupMenuItem(
          onTap: _addToMusicPlaylist,
          child: const Text('添加到音乐播放列表'),
        ),
  ...
  void _addToMusicPlaylist() {
    if (!videoDetailController.isUgc) {
      SmartDialog.showToast('暂不支持此类型视频');
      return;
    }

    final videoDetail = ugcIntroController.videoDetail.value;
    final pages = videoDetail.pages;
    final ugcSeason = videoDetail.ugcSeason;

    // 检查是否有多P或合集
    final hasMultiParts = pages != null && pages.length > 1;
    final hasSeason = ugcSeason != null && ugcSeason.sections != null;

    if (hasMultiParts || hasSeason) {
      // 显示选择对话框
      _showBatchImportDialog(videoDetail, pages, ugcSeason);
    } else {
      // 直接添加当前视频
      _addCurrentVideoToPlaylist();
    }
  }

  void _addCurrentVideoToPlaylist() {
    try {
      ...
      SmartDialog.showToast('已添加到音乐播放列表');
    } catch (e) {
  ...
  void _showBatchImportDialog(
    ...
    showModalBottomSheet(
      ...
      builder: (context) => _BatchImportSheet(
...
class _BatchImportSheetState extends State<_BatchImportSheet> {
  ...
  bool isSelectingParts = true; // true = 分P, false = 合集
  int currentSeasonIndex = 0;

  List<dynamic> get currentList {
    if (isSelectingParts && widget.pages != null) {
      return widget.pages!;
    } else if (!isSelectingParts && widget.ugcSeason?.sections != null) {
      final section = widget.ugcSeason!.sections![currentSeasonIndex];
  ...
  void initState() {
    ...
    if (widget.pages != null && widget.pages!.isNotEmpty) {
      final currentCid = widget.videoDetailController.cid.value;
      for (int i = 0; i < widget.pages!.length; i++) {
        if (widget.pages![i].cid == currentCid) {
  ...
  @override
  Widget build(BuildContext context) {
    final hasPages = widget.pages != null && widget.pages!.length > 1;
    final hasSeason = widget.ugcSeason?.sections != null;
      ...
      builder: (context, scrollController) {
        return Column(
          children: [
            ...
            _buildHeader(),
            if (hasPages && hasSeason) _buildTabBar(),
      ...
      child: Row(
        ...
        children: [
          ...
          Row(
            children: [
              TextButton(
                onPressed: selectedIndices.isEmpty ? null : _importSelected,
                child: Text('导入(${selectedIndices.length})'),
              ),
            ],
          ),
        ],
  ...
  Widget _buildTabBar() {
    return Padding(
      ...
      child: Row(
        children: [
          _buildTabButton('分P', isSelectingParts, () {
            setState(() {
          ...
          _buildTabButton('合集', !isSelectingParts, () {
            setState(() {
  ...
  Widget _buildSelectAllRow() {
    ...
    final allSelected =
        selectedIndices.length == list.length && list.isNotEmpty;
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Text(
            '全选 (${list.length}个${isSelectingParts ? "分P" : "视频"})',
  ...
  Widget _buildList(ScrollController scrollController) {
    ...
    if (list.isEmpty) {
      return Center(
        child: Text(
          '暂无内容',
          style: TextStyle(color: widget.theme.colorScheme.outline),
        ),
      );
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (item is Part) {
          title = 'P${index + 1} ${item.part ?? ""}';
          if (item.duration != null) {
            final dur = Duration(seconds: item.duration!);
            duration =
                '${dur.inMinutes}:${(dur.inSeconds % 60).toString().padLeft(2, '0')}';
          }
        } else if (item is ugc.EpisodeItem) {
          title = item.title ?? 'P${index + 1}';
          if (item.arc?.duration != null) {
            final dur = Duration(seconds: item.arc!.duration!);
            duration =
                '${dur.inMinutes}:${(dur.inSeconds % 60).toString().padLeft(2, '0')}';
          }
        } else {
        ...
        return CheckboxListTile(
          ...
          title: Text(
            ...
            overflow: TextOverflow.ellipsis,
            style: TextStyle(
              color: isSelected ? widget.theme.colorScheme.primary : null,
            ),
          ),
          subtitle: duration != null ? Text(duration) : null,
        );
      },
    );
  }
  ...
  void _importSelected() {
    ...
    try {
      ...
      final list = currentList;
      final title = widget.videoDetail.title ?? '';
      final owner = widget.videoDetail.owner?.name ?? '';
      final cover = widget.videoDetailController.cover.value;
      final bvid = widget.videoDetailController.bvid;
      final aid = widget.videoDetailController.aid;
      ...
      for (final index in selectedIndices.toList()..sort()) {
        ...
        if (item is Part) {
          itemTitle = '$title - P${index + 1} ${item.part ?? ""}';
          cid = item.cid;
        } else if (item is ugc.EpisodeItem) {
          itemTitle = item.title ?? '$title - P${index + 1}';
          cid = item.cid;
        } else {
          continue;
        }
      ...
      SmartDialog.showToast('已添加$addedCount个视频到播放列表');
    } catch (e) {
      SmartDialog.showToast('添加失败: $e');
    }
  }
