...
import 'package:PiliPlus/plugin/pl_player/widgets/danmaku_settings_panel.dart';
...
class _PLVideoPlayerState extends State<PLVideoPlayer>
  ...
  // 单击快进/快退冷却时间
  DateTime? _lastTapSeekTime;

  // 长按滑动全屏手势
  bool _isLongPressing = false;
  double? _longPressStartY;
  ...
  Widget buildBottomControl(
    ...
    Widget progressWidget(
      ...
      /// 弹幕设置
      BottomControlType.danmakuSettings => ComBtn(
        width: widgetWidth,
        height: 30,
        tooltip: '弹幕设置',
        icon: Stack(
        ...
        onTap: () {
          _showDanmakuSettingsPanel(context);
        },
      ),
    ...
    List<BottomControlType> userSpecifyItemRight = [
      ...
      if (isNotFileSource) BottomControlType.danmakuSettings,
    ...
    switch (details.kind) {
      ...
      default:
        if (_suspendedDm == null) {
          // 全屏模式下使用分区单击功能（需要开启设置）
          if (isFullScreen &&
              !plPlayerController.controlsLock.value &&
              GStorage.setting.get(
                SettingBoxKey.enableTapSeek,
                defaultValue: false,
              )) {
            final double tapPosition = details.localPosition.dx;
            final double leftSection = maxWidth / 4; // 左侧1/4
            final double rightSection = maxWidth * 3 / 4; // 右侧1/4起点

            // 检查冷却时间（1.5秒防误触）
            final now = DateTime.now();
            if (_lastTapSeekTime != null &&
                now.difference(_lastTapSeekTime!).inMilliseconds < 1500) {
              return;
            }

            if (tapPosition < leftSection) {
              // 单击左侧1/4：后退15秒
              _lastTapSeekTime = now;
              plPlayerController.onBackward(const Duration(seconds: 15));
              SmartDialog.showToast(
                '快退15s',
                displayTime: const Duration(milliseconds: 800),
              );
            } else if (tapPosition >= rightSection) {
              // 单击右侧1/4：快进15秒
              _lastTapSeekTime = now;
              plPlayerController.onForward(const Duration(seconds: 15));
              SmartDialog.showToast(
                '快进15s',
                displayTime: const Duration(milliseconds: 800),
              );
      ...
      LongPressGestureRecognizer(
        ...
        ..onLongPressStart = ((details) {
          ...
          if (GStorage.setting.get(
            SettingBoxKey.enableLongPressFullscreen,
            defaultValue: false,
          )) {
            _isLongPressing = true;
            _longPressStartY = details.localPosition.dy;
          }
        ...
        ..onLongPressMoveUpdate = ((details) {
          ...
          if (_isLongPressing &&
            ...
            final deltaY = details.localPosition.dy - _longPressStartY!;
            // 滑动超过50像素触发
            if (deltaY.abs() > 50) {
