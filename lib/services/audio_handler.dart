...
class VideoPlayerServiceHandler extends BaseAudioHandler with SeekHandler {
  ...
  // 音乐播放器专用的媒体项
  MediaItem? _musicPlayerItem;
  bool _isMusicPlayerActive = false;
  ...
  // 音乐播放器专用：设置媒体项
  Future<void> setMusicPlayerMediaItem(MediaItem newMediaItem) async {
    if (!enableBackgroundPlay) return;
    _musicPlayerItem = newMediaItem;
    _isMusicPlayerActive = true;
    if (!mediaItem.isClosed) mediaItem.add(newMediaItem);
  }

  // 清除音乐播放器状态
  void clearMusicPlayerState() {
    _musicPlayerItem = null;
    _isMusicPlayerActive = false;
  }
  ...
  // 音乐播放器专用的播放状态（带上一首/下一首）
  Future<void> setMusicPlaybackState(
    PlayerStatus status,
    bool isBuffering,
    ...
    playbackState.add(
      playbackState.value.copyWith(
        processingState: isBuffering
            ...
            : processingState,
        controls: [
          if (hasMultipleTracks) MediaControl.skipToPrevious,
          if (playing) MediaControl.pause else MediaControl.play,
          if (hasMultipleTracks) MediaControl.skipToNext,
        ],
        playing: playing,
        systemActions: const {
          MediaAction.seek,
  ...
  void onPositionChange(Duration position) {
    ...
    // 支持音乐播放器的位置更新
    if (_isMusicPlayerActive && _musicPlayerItem != null) {
      playbackState.add(
        playbackState.value.copyWith(
          updatePosition: position,
        ),
    ...
    if (_item.isEmpty || !PlPlayerController.instanceExists()) {
      return;
    }
